...
from src.data.als_loading import load_als_survey, load_raster
...
def compute_recovery_dataset(
    ...
    relevant_datasets = ("AnnualChange", "DeforestationYear", "DegradationYear")
    jrc_data = {
        dataset: load_jrc_data(
            survey.lon_min,
            survey.lat_min,
            survey.lon_max,
            survey.lat_max,
    ...
    chm_data = {
        name: chm.rio.reproject_match(als_data["mean"], resampling=method).squeeze()
    ...
    for key, val in chm_data.items():
        final_data[f"chm_{key}"] = val.where(recovery_period > 0).data.flatten()
    ...
    for key, val in als_data.items():
        ...
        if resolution == extra_als_datasets["mean"]:
            ...
            final_data[key] = (
                val.rio.reproject_match(recovery_period, resampling=Resampling.sum)
    ...
    x_ind, y_ind = np.meshgrid(
        np.arange(0, len(recovery_period.x)), np.arange(0, len(recovery_period.y))
    )
